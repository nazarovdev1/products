...
setInterval(() => {
  fetch("https://products-h304.onrender.com") // o'zgartiring
    ...
    upload.single("productImage")(req, res, (err) => {
      if (err instanceof multer.MulterError) {
        if (err.code === "LIMIT_FILE_SIZE") {
          return res
            .status(400)
            .json({ error: "Rasm hajmi juda katta. Maksimum 10MB" });
        ...
        return res
          .status(400)
          .json({ error: "Rasm yuklashda xatolik: " + err.message });
      }
      next();
    });
  },
...
app.delete("/api/products/:code", async (req, res) => {
  try {
    ...
    console.log(
      "Available products:",
      products.map((p) => ({ code: p.code, title: p.title }))
    ); // Debug log
